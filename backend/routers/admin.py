...
def get_current_admin(credentials: HTTPBasicCredentials = Depends(security)):
    correct_username = secrets.compare_digest(credentials.username, "admin@bookshelf.com")
...
async def upload_books(
    ...
    for epub_file in epubs:
        if not epub_file.filename.lower().endswith(".epub"):
            raise HTTPException(status_code=400, detail=f"'{epub_file.filename}' is not an EPUB file.")
        ...
        if existing_book:
            ...
            continue
        book = Book(title=title, author=author, epub_filepath=epub_path, cover_filepath=cover_filepath)
        db.add(book)
        db.commit()
        db.refresh(book)
        results.append(book)
...
def delete_book(book_id: int, db: Session = Depends(get_db), admin: str = Depends(get_current_admin)):
    ...
    try:
        if book.epub_filepath and os.path.exists(book.epub_filepath): os.remove(book.epub_filepath)
        if book.cover_filepath and os.path.exists(book.cover_filepath): os.remove(book.cover_filepath)
...
def get_admin_stats(db: Session = Depends(get_db), admin: str = Depends(get_current_admin)):
    ...
    for b in books:
        ...
        books_stats.append(BookStats(book_id=b.id, title=b.title, total_sends=sends, unique_users=uniques or 0))
    return AdminStatsResponse(total_books=total_books, total_sends=total_sends, books_stats=books_stats)

@router.get("/contributions", response_model=List[BookResponse])
def get_pending_contributions(db: Session = Depends(get_db), admin: str = Depends(get_current_admin)):
    return db.query(Book).filter(Book.status == "pending").all()

@router.post("/contributions/{book_id}/approve")
def approve_contribution(book_id: int, db: Session = Depends(get_db), admin: str = Depends(get_current_admin)):
...
def reject_contribution(book_id: int, db: Session = Depends(get_db), admin: str = Depends(get_current_admin)):
    ...
    db.commit()
    return {"message": "Contribution rejected and moved to history."}
